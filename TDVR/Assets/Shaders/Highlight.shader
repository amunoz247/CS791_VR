//======= Modified from Valve Corporation ===============
//
...
Shader "Highlight"
{
	...
	Properties
	{
		...
		g_flExtrudeDistance( "Outline Distance", Range ( 0, .03 ) ) = 0
	}
	...
	CGINCLUDE
		...
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------
		PS_INPUT Extrude( PS_INPUT vertex, float extrudeDistance )
		{
...
#endif
			extruded.vPositionPs.xy += vOffsetPs.xy * extruded.vPositionPs.w * extrudeDistance;
		...
		//-------------------------------------------------------------------------------------------------------------------------------------------------------------
		[maxvertexcount(18)]
		void ExtrudeBlankGs( triangle PS_INPUT inputTriangle[3], inout TriangleStream<PS_INPUT> outputStream )
		{
		    PS_INPUT extrudedTriangle0 = Extrude( inputTriangle[0], g_flExtrudeDistance );
		    PS_INPUT extrudedTriangle1 = Extrude( inputTriangle[1], g_flExtrudeDistance );
		    PS_INPUT extrudedTriangle2 = Extrude( inputTriangle[2], g_flExtrudeDistance );
		...
		{
		    PS_INPUT extrudedTriangle0 = Extrude( inputTriangle[0], g_flExtrudeDistance + g_flOutlineWidth );
		    PS_INPUT extrudedTriangle1 = Extrude( inputTriangle[1], g_flExtrudeDistance + g_flOutlineWidth );
		    PS_INPUT extrudedTriangle2 = Extrude( inputTriangle[2], g_flExtrudeDistance + g_flOutlineWidth );
	...
	SubShader
	{
		...
		Pass
		{
			...
			CGPROGRAM
				...
				#pragma geometry ExtrudeBlankGs
